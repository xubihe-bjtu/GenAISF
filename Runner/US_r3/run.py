...
parser.add_argument('--data_path', type=str, default='../../datasets/US_r3', help='Input Data Path')
...
parser.add_argument('--train_type',type=str,default='Normal',help='Kriging|Normal|ST|STKri')
parser.add_argument('--save_path', type=str, default='../../save/US_r3', help='Result Save File Path')
...
parser.add_argument('--M', type=int, default=10, help='Number of Runs')
parser.add_argument('--batch_size', type=int, default=8)
parser.add_argument('--k', type=int, default=6,help='k Nearest Neighbors for Proxy Station')
parser.add_argument('--area',type=str,default='US_r3',help='region')
...
parser.add_argument('--model',type=str,default='GeneAIS',help='model type')
...
parser.add_argument('--num_nodes', type=int, default= 366,help='Number of Observation Stations')
...
parser.add_argument('--gamma',default=0.01,type=float,help='loss weight')
parser.add_argument('--train_ratio',default=0.7,type=float,help='loss weight')
parser.add_argument('--param_test', type=str, default='False', help='Model Path')
parser.add_argument('--param_test_lambda', type=str, default='False', help='Model Path')
...
if args.train_type=='STKri':
    ...
    if args.param_test == 'True':
        run_dir = create_save_path(args.save_path, args.model + f'/M/{args.M}', args.train_type)
    elif args.param_test_lambda == 'True':
        run_dir = create_save_path(args.save_path, args.model + f'/lambda/{args.gamma}', args.train_type)
    else:
...
if args.train_type=='ST':
    ...
    if args.param_test == 'True':
        save_experiment_result(args.model + f'/M/{args.M}', args.ST_model, target_name, yhat_data, args.save_path,
                               args.train_type)
    elif args.param_test_lambda=='True':
        save_experiment_result(args.model + f'/lambda/{args.gamma}', args.ST_model, target_name, yhat_data, args.save_path,
                               args.train_type)
    else:
        save_experiment_result(args.model, args.ST_model, target_name, yhat_data, args.save_path, args.train_type)
